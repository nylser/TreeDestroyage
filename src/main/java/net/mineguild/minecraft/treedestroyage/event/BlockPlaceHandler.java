...
public class BlockPlaceHandler {
    ...
    public BlockPlaceHandler(TreeDestroyage plugin){
        ...
        for (File file:worlds){
            Optional<World> optw = Sponge.getServer().getWorld(file.getName().replace(".txt",""));
            if (!optw.isPresent()) continue;
            try{
                ...
                reader.lines().filter(Objects::nonNull).forEach(line -> {
                    ...
                    Location<World> loc = new Location<>(optw.get(),x,y,z);
    ...
    private void Save(String world){
        try{
            ...
            for (Location<World> loc:placedBlocks.keySet().stream().filter(l -> l.getExtent().getName().equals(world)).collect(Collectors.toList())){
                long now = Calendar.getInstance().getTime().getTime();
                long time = placedBlocks.get(loc);

                long days = TimeUnit.DAYS.convert(now - time, TimeUnit.MILLISECONDS);
                int pday = plugin.getConfig().getNode("purgeBlocksTime").getInt(-1);
                if (pday < 0 || days <= pday){
                    int x = loc.getBlockX();
                    int y = loc.getBlockY();
                    int z = loc.getBlockZ();
